...
[观看 12 秒无声功能导览](../assets/demo/datastoria-tour.mp4)

## 1. 首次进入

1. 打开管理员提供的 DataStoria 地址；
2. 生产环境通过组织 OAuth/OIDC 登录；
3. 本地开发环境默认使用 `dev@example.com`；
4. 首次进入且没有连接时，页面会引导创建 ClickHouse Connection。

![DataStoria 主界面](../assets/screenshots/workbench-overview.jpg)
...
![创建 ClickHouse 连接](../assets/screenshots/connection-dialog.jpg)
...
![ClickHouse 查询 Dashboard](../assets/screenshots/dashboard-query.jpg)
...
![模型与供应商设置](../assets/screenshots/model-settings.jpg)
...
4. 对需要授权的动作选择 Approve 或 Deny；
5. 遇到补充问题时填写答案，服务端通过 Action API 恢复原 Run；
6. 网络中断后页面使用事件序号重连，不要重复提交同一问题。
...
更多错误见[故障排查](../operations/troubleshooting.md)。
