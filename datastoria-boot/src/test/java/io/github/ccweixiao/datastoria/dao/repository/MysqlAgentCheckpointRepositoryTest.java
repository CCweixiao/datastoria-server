...
/**
 * MySQL contract for {@link AgentCheckpointRepository}: append (new sequence), overwrite (same
 * sequence preserves created_at, bumps updated_at), latest-by-max-sequence, ordered reads, and
 * tenant isolation.
 */
@SpringBootTest
@ActiveProfiles("dev")
class MysqlAgentCheckpointRepositoryTest {
  ...
  @Test
  void saveOverwritesAtSameSequencePreservingCreatedAt() throws Exception {
    ...
    assertThat(JSON.readTree(first.stateJson())).isEqualTo(JSON.readTree("{\"v\":1}"));
    ...
    assertThat(JSON.readTree(overwritten.stateJson())).isEqualTo(JSON.readTree("{\"v\":99}"));
  ...
  void concurrentSaveAtSameSequenceConvergesWithoutUniqueKeyFailure() throws Exception {
    ...
    assertThat(JSON.readTree(checkpoints.get(0).stateJson()).path("writer").asInt()).isIn(1, 2);
  }
