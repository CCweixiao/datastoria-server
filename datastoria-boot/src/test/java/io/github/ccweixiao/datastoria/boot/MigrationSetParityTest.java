...
class MigrationSetParityTest {
  ...
  void deploymentSnapshotsContainTheSameTablesAsMigrations() throws Exception {
    for (String dialect : Set.of("sqlite", "mysql")) {
      ...
      Set<String> snapshotTables =
          createdTables(
              java.util.List.of(
                  Path.of("datastoria-dao/src/main/resources/db/schema", dialect, "schema.sql")));
      ...
      assertThat(
              Files.readString(
                  Path.of("datastoria-dao/src/main/resources/db/schema", dialect, "seed.sql")))
  ...
  private Map<String, Path> migrations(String dialect) throws Exception {
    Path directory = Path.of("datastoria-dao/src/main/resources/db/migration", dialect);
