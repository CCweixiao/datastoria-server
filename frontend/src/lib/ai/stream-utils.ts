...
export async function generateObject<SCHEMA extends FlexibleSchema<unknown>>(input: {
  model: LanguageModel;
  prompt: string;
  system?: string;
  schema: SCHEMA;
  temperature?: number;
  supportsStructuredOutputs: boolean;
}): Promise<InferSchema<SCHEMA> & { usage: LanguageModelUsage }> {
  ...
  const output = input.supportsStructuredOutputs
    ? (result.output as InferSchema<SCHEMA>)
    : parseTextObjectResponse(result.text, input.schema);
...
export async function streamObjectWithMessages<SCHEMA extends FlexibleSchema<unknown>>(input: {
  model: LanguageModel;
