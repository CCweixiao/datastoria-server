...
import { generateObject as generateObjectFromUtils } from "@/lib/ai/stream-utils";
...
async function classifyByLLM(input: InputMessages, modelConfig: InputModel): Promise<PlanResult> {
  ...
  const plannerPrompt = new PlannerPromptBuilder()
    ...
    .build();
  try {
    const result = await generateObjectFromUtils({
      model,
    ...
    if (!result) {
      throw new Error("No output generated from generateObject");
    }

    const { usage } = result;
    const { intent, reasoning } = result;
    let { title } = result;

    if (input.isFirstUserMessage) {
      if (!title || title.trim() === "") {
        title = generateTitleFromUIMessage(lastUser);
      }
    }
    ...
    return {
      ...
      reasoning,
      title,
      agent: SUB_AGENTS[intent as Intent] ?? SUB_AGENTS.general,
      usage,
    };
  } catch {
    return {
