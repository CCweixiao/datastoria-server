...
import { streamObjectWithMessages } from "../stream-utils";
...
2. Don't wrap the JSON in any additional text or formatting.`;
  ...
  try {
    ...
    const validated = await streamObjectWithMessages({
      model,
      messages: [{ role: "system", content: systemPrompt }, ...messages] as ModelMessage[],
