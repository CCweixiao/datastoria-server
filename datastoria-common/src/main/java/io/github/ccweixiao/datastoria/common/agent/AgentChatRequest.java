 ...
 * harness runtime knobs (loop bound, eviction, compaction) are never request-settable — they come
 * from {@code datastoria.agent.*} defaults merged with tenant-level admin overrides. Safe
 * diagnostic fields from {@code context} are added to the server-owned system prompt. {@code
