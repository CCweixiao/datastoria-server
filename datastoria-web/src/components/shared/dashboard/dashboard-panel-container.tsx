...
import { connect, type ECharts } from "echarts";
import React, {
  createContext,
  forwardRef,
  useCallback,
...
>(
  (
    ...
    const connectAllCharts = useCallback(() => {
      const chartInstances: ECharts[] = subComponentRefs.current
        .filter(
          (chartRef): chartRef is DashboardVisualizationComponent =>
            chartRef !== null &&
            typeof (chartRef as unknown as { getEChartInstance?: () => ECharts })
              .getEChartInstance === "function"
        )
        .map((chartRef) => {
          const component = chartRef as unknown as { getEChartInstance: () => ECharts };
