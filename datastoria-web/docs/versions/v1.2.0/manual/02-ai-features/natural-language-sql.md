...
![聊天面板](../../en/manual/02-ai-features/img/chat-panel.png)

面板提供了若干示例问题，帮助你快速上手。

2. 在 INPUT 区域的模型选择列表中选择一个模型
![模型选择](../../en/manual/02-ai-features/img/select-model.png)
...
![AI 聊天响应：展示生成的用于获取 ClickHouse 版本的 SQL 查询及执行结果](../../en/manual/02-ai-features/img/chat-example-1.png)
...
![自然语言查询结果：展示过去 3 小时内 system.query_log 中 SELECT 查询的数量](../../en/manual/02-ai-features/img/chat-example-2.jpg)
...
![AI 生成的查询：找出过去 6 小时内查询日志中最慢的 SELECT 查询及其执行时间详情](../../en/manual/02-ai-features/img/chat-example-slowest-query.jpg)
...
![自然语言查询：展示 2021 年 2 月 git_clickhouse.commits 表中前 3 名提交者及表格形式的结果](../../en/manual/02-ai-features/img/chat-example-3.jpg)

在本例中，由于 LLM 并不知道表结构，它首先使用 `explore_schema` 工具检查表结构，以决定 SQL 中要使用哪些列。我们可以展开该工具查看其输入和输出。

![AI explore_schema 工具输出：展示用于 SQL 生成的表结构和列信息](../../en/manual/02-ai-features/img/chat-example-3-table-schema.jpg)
...
![追问查询：使用 AI 生成的 CTE SQL 比较 2021 年 1 月与 2 月的新增提交者](../../en/manual/02-ai-features/img/chat-example-4.jpg)
