...
![SQL 编辑器界面，显示类终端的查询输入区（带语法高亮），查询结果显示在上方](../../../manual/03-query-experience/img/sql-editor-1.jpg)
...
- 当前数据库中可用的表
  ![自动补全下拉列表，显示当前数据库中可用的表建议](../../../manual/03-query-experience/img/sql-editor-suggestion-1.jpg)

- 当前表的列
  ![自动补全显示所选表的列建议及数据类型](../../../manual/03-query-experience/img/sql-editor-suggestion-2.jpg)

### ClickHouse 设置（Settings）建议

当你输入 `SETTINGS`、`SET` 或 `settings`（不区分大小写）时，编辑器会自动建议所有可用的 ClickHouse 设置，并附带其当前值和描述。

![ClickHouse 设置自动补全，显示可用设置及其当前值和描述](../../../manual/03-query-experience/img/sql-editor-suggestion-3.jpg)

### ClickHouse 输入/输出格式建议

当你输入 `FORMAT` 或 `format`（不区分大小写）时，编辑器会建议你的 ClickHouse 实例支持的所有输入和输出格式。

![FORMAT 子句自动补全，显示所有受支持的 ClickHouse 输入和输出格式](../../../manual/03-query-experience/img/sql-editor-suggestion-4.jpg)

### ClickHouse 表引擎

当你在 `ENGINE` 关键字后输入 `=` 时，编辑器会建议你的 ClickHouse 实例支持的所有表引擎。

![表引擎自动补全，显示 ENGINE 关键字之后可用的 ClickHouse 表引擎](../../../manual/03-query-experience/img/sql-editor-suggestion-5.jpg)
...
![sql-editor-suggestion-6](../../../manual/03-query-experience/img/sql-editor-suggestion-6.jpg)
