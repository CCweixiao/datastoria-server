...
import { PROVIDER_GLM, PROVIDER_NEBIUS, PROVIDER_OPENAI_CODEX } from "./provider-ids";
...
describe("structured output support", () => {
  it("uses text JSON fallback only for providers and models that need it", () => {
    expect(LanguageModelProviderFactory.supportsStructuredOutputs(PROVIDER_GLM, "GLM-5.1")).toBe(
      false
    );
    expect(LanguageModelProviderFactory.supportsStructuredOutputs("GitHub Copilot", "gpt-4o")).toBe(
      false
    );
    expect(LanguageModelProviderFactory.supportsStructuredOutputs("GitHub Copilot", "gpt-5")).toBe(
      true
    );
    expect(LanguageModelProviderFactory.supportsStructuredOutputs("OpenAI", "gpt-5")).toBe(true);
  });
});
