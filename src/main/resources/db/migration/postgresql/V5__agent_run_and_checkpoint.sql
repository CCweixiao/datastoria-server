-- V5__agent_run_and_checkpoint.sql (PostgreSQL dialect)
--
...
--   * JSON columns use JSON validity CHECK; MySQL relies on its native json type (see mysql dialect).
...
CREATE TABLE ds_agent_run (
    ...
    request_id              TEXT,
    connection_id           TEXT,
    input_snapshot_json     TEXT CHECK (input_snapshot_json IS NULL OR (input_snapshot_json::jsonb IS NOT NULL)),
    usage_json              TEXT CHECK (usage_json IS NULL OR (usage_json::jsonb IS NOT NULL)),
    error_code              TEXT,
    safe_message            TEXT,
...
CREATE TABLE ds_agent_checkpoint (
    ...
    state_json      TEXT NOT NULL CHECK ((state_json::jsonb IS NOT NULL)),
