 ...
 * cursor wire format ({@link io.datastoria.server.repository.SessionListCursor}), which is
 * millisecond precise, so SQLite's lexicographic TEXT comparison never re-includes the cursor's own
 * row on the next page. It also keeps sort order and cursor semantics identical across SQLite and
 * MySQL.
 *
...
public class InstantTypeHandler extends BaseTypeHandler<Instant> {
  ...
  public void setNonNullParameter(PreparedStatement ps, int i, Instant parameter, JdbcType jdbcType)
      throws SQLException {
    ps.setString(i, parameter.truncatedTo(ChronoUnit.MILLIS).toString());
  }
