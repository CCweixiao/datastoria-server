...
import io.datastoria.server.persistence.entity.AgentPendingActionEntity;
import io.datastoria.server.persistence.mapper.AgentPendingActionMapper;
  ...
  public AgentPendingAction resolve(
    ...
    AgentPendingAction raced =
        Optional.ofNullable(mapper.findById(tenantId, userId, runId, actionId))
            .map(AgentPendingActionEntity::toDomain)
            .orElseThrow();
    if (raced.status() == io.datastoria.server.agent.domain.PendingActionStatus.EXPIRED) {
      throw new PendingActionExpiredException(actionId);
    }
